 ...
 * Date: 16/06/13 22:27
 *
 * (C) Copyright Tiny MVC 2013 - All rights reserved
 */
...
class SecurityComponent extends Component{
	...
	/**
	 * Hashes a string using salts and returns it
	 *
	 * @param $str
	 * @return string
	 */
	public function hash($str) {
		$salt1 = substr(Config::SALT, 0, 15);
		$salt2 = substr(Config::SALT, strlen(Config::SALT)-15);

		$str = hash('sha256', md5($salt1).$str.md5($salt2));

		return $str;
	}

	/**
	 * Encodes html characters to their html equal and returns the encoded content
	 * Takes an array or a string as parameter
	 *
	 * @param mixed $content
	 * @return array|string
	 */
	public function xss($content) {
		if (is_array($content)) {
			foreach($content as $i => $item) {
				if (!is_array($item)) {
					$content[$i] = htmlspecialchars($item);
				}
				else {
					$content[$i] = $this->xss($item);
				}
			}
		}
		else {
	...
	/**
	 * Escapes SQL-sensitive characters and returns the escaped content
	 * Takes an array or a string as parameter
	 *
	 * @param mixed $content
	 * @return array|string
	 */
	public function sqlInjection($content) {
		if (is_array($content)) {
			foreach($content as $i => $item) {
				if (!is_array($item)) {
					$content[$i] = addslashes($item);
				}
				else {
					$content[$i] = $this->xss($item);
				}
	...
	/**
	 * Returns an xss and sql injection protected $_POST array
	 *
	 * @param $post
	 * @return array|string
	 */
	public function getPost($post) {
		return $this->xss($this->sqlInjection($post));
	}
	...
	public function getGet($get) {
		return $this->xss($this->sqlInjection($get));
	}

	/**
	 * Encrypt a String using the TEA-algorithm
	 *
	 * @param $str
	 * @return string
	 */
	public function encrypt($str) {
		...
		$salt = md5($salt1.'C0M-uN1tY'.$salt2);
		$xtea = new xtea($salt);

		return $xtea->encrypt($str);
	}

	/**
	 * Decrypt a String which has been decrypted using the TEA-algorithm
	 *
	 * @param $str
	 * @return string
	 */
